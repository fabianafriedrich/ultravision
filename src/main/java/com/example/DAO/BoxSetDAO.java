...
@Repository
public interface BoxSetDAO extends JpaRepository<BoxSet, Long >{
    /*HQL it means sequal sql from hibernate*/
    @Query("SELECT bs FROM BoxSet bs WHERE bs.title  =:name")
    List<BoxSet> findByName(@Param("name") String name);
}
